...
class Safe:
    ...
    def turn_dial(self, rotation: Rotation) -> "Safe":
        ...
        crossed: bool = (raw_new_dial_point < 0) or (raw_new_dial_point > 99)
        new_dial_point: int = raw_new_dial_point % 100
        return Safe(dial_point=new_dial_point, crossed=crossed)
...
class PasswordDiscoverState:
    ...
    def execute_rotation(self, rotation: Rotation) -> "PasswordDiscoverState":
        circles: int = rotation.clicks // 100
        simplified_rotation = Rotation(
            direction=rotation.direction, clicks=rotation.clicks % 100
        )
        new_safe_state: Safe = self.safe_state.turn_dial(rotation=simplified_rotation)
        new_how_many_times_dial_zero: int = self.how_many_times_dial_zero

        if self.safe_state.dial_point == 0:
            new_how_many_times_dial_zero += circles

        elif new_safe_state.dial_point == 0:
            new_how_many_times_dial_zero += circles + 1

        else:
            new_how_many_times_dial_zero += circles + int(new_safe_state.crossed)
